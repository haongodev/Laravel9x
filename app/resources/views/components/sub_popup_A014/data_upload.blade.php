<tr class="reflectionsheet-id-{{$reflectionSheetManager->id}}">
    <td style="width: 100px">
        <div
            class="share-reflectionsheet manager {{$reflectionSheetManager->share_flg ? 'sharing btn-eff-pri' : 'btn-eff-ora'}} btn-hov"
    ...
    <td>
        ...
        <div class="manager btn-eff-ora btn-hov"><a download class="download" href="{{config('constants.path_upload').'/'.auth()->user()->user_add_info->login_id.'/reflectionsheet/'.$pathClass.'/'.$reflectionSheetManager->file_name}}">{{$reflectionSheetManager->display_name}}</a></div>
    </td>
    <td style="width: 100px">
