<tr class="a015-initive-list initiativetable-id-{{$initiativetableManager->id}}">
    ...
    <td>
        <div class="manager btn-eff-ora btn-hov"><a download class="download" href="{{ asset('storage/upload/'.auth()->user()->user_add_info->login_id.'/initiative/'.$initiativetableManager->file_name) }}">{{$initiativetableManager->display_name}}</a></div>
    </td>
