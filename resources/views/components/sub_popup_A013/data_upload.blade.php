<tr class="facesheet-id-{{$faceSheetManager->id}}">
    <td class="w-100px" >
        <div class="share-facesheet manager {{$faceSheetManager->share_flg ? 'share btn-eff-pri' : 'btn-eff-ora'}}"
    ...
    <td>
        <div class="manager btn-eff-ora"><a download class="download" href="{{config('constants.path_upload').'/'.auth()->user()->user_add_info->login_id.'/facesheet/'.$faceSheetManager->file_name}}">{{$faceSheetManager->display_name}}</a></div>
    </td>
